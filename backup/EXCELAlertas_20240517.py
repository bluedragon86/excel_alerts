...
def add_email_list():
    # Adiciona registo
    new_email = simpledialog.askstring("Adicionar email", "Endereço de email:")

    if new_email:
       email_list.insert(END, new_email)

def del_email_list():
    # Apaga registo
    if email_list.curselection():
        index = email_list.curselection()
        email_list.delete(ANCHOR)
...
def update(id_registro):
    ...
    c.execute("""UPDATE registros 
                        ...
                        id=:id""",
                {
                        ...
                        'emails_value': serialized_emails,
                        'id': id_registro
                } 
                )
...
def on_checkbox_checked():
   # global checkbox_value
    if checkbox_var.get() == 1:
        if not email_list.size():
            messagebox.showerror("Erro", "Lista de emails vazia.")
            checkbox_var.set(0)
...
def execute():
    ...
    if matriz:
        ...
        messagebox.showinfo("Information", "0 linhas enviadas.")
...
def form_details(parent, registro):
    ...
    botton_frame_right.grid(row=0, column=1, sticky="n")

    botton_frame_right1 = Frame(botton_frame, padx=10, pady=10)
    botton_frame_right1.grid(row=0, column=2, sticky="n")
...
###################################### Top frame #########################################
    ...
    columns_list_label.grid(row=0, column=0, padx=8, sticky="w")

    columns_list = Listbox(botton_frame_right, width=20, height=25)
    columns_list.grid(row=1, column=0, padx=10, sticky="sn")


    btn_del_email = Button(botton_frame_right1, text="Eliminar", width=34, command=del_email_list)
    btn_del_email.pack(side=BOTTOM, anchor="nw", pady=0)

    # Botão de adicionar email
    btn_add_email = Button(botton_frame_right1, text="Adicionar", width=34, command=add_email_list)
    btn_add_email.pack(side=BOTTOM, anchor="nw", pady=10)
    #btn_add_email.grid(row=3, column=0, sticky="w")


    global email_list
    email_list_label = Label(botton_frame_right1, text="Lista de emails:")
    email_list_label.pack(side=TOP, anchor="nw")
    #email_list_label.grid(row=0, column=1, padx=27, sticky="wn")

    email_list = Listbox(botton_frame_right1, width=41)
    email_list.pack(side=LEFT)
   # email_list.grid(row=1, column=1, padx=30, sticky="wn")
...
def query(registro):
    ...
    # Campo emails
    if registro[12]:
        stored_emails = json.loads(registro[12])
